...
- **Справочник Kookaburra:** структура подсерий с [pixelur.com/Kookaburra](https://www.pixelur.com/Kookaburra.html) выписана в `docs/PIXELUR_KOOKABURRA.md` — можно использовать для единообразного поля `series` при импорте Perth Mint.
- **Отложено (вернуться позже): коллекция и удалённые монеты.** Если пользователь добавил монету в коллекцию, а мы потом удалили её из каталога (например, дубль) — запись в Supabase `user_collection` не удаляется; в UI портфолио эта монета не показывается (fetch по `/data/coins/<id>.json` даёт 404, строка не попадает в таблицу). В БД остаются «висячие» id. Идея на потом: при загрузке портфолио по 404 удалять такие записи из `user_collection`.
...
**Локальный `npm run dev`**: в `package.json` задано `next dev --webpack` (Webpack), а не Turbopack по умолчанию. Так стабильнее на части машин и при путях проекта с кириллицей — иначе Next 16 иногда показывает «unexpected Turbopack error». Продакшен-сборка `npm run build` не меняется. Если нужен именно Turbopack в dev: `npm run dev:turbo`. После смены режима при странностях удалите кэш: `rm -rf .next`.
...
- **Сколько Perth и откуда дубликаты**: ожидается ~965 монет Perth (по каноникам с source_url). Диагностика: `node scripts/count-perth-and-duplicates.js` — число записей Perth, уникальных source_url, группы «один URL — несколько записей». Возможные причины дублей: (1) импорт одного продукта из нескольких файлов (разный slug → разный catalog_number); (2) повторный импорт, когда по catalog_number несколько записей — обновлялась одна, для «того же» продукта мог создаться новый INSERT; (3) один продукт в двух файлах с разным source_url; (4) после массовой перезаписи часть записей получила тот же source_url, что и уже существовавшие. Удалять только безопасные дубликаты (совпадение source_url + спеки).
- **Дубликаты монет — комплексная проверка**: совпадение title + catalog_number может быть из‑за перезаписи (разные монеты получили одни данные), а не реального дубликата. Поэтому перед удалением «дублей» проверяем шире: сравниваем **source_url**, вес (weight_g), диаметр (diameter_mm), металл (metal) и при необходимости изображения. Удалять только если **все** ключевые поля совпадают. Скрипты: `scripts/check-duplicate-coins-safe.js` (отчёт: безопасные дубликаты vs подозрительные группы с разными спеками), `scripts/remove-duplicate-coins.js` (удаляет только безопасные дубликаты).
...
- **Карточка каталога / детали:** если у конкретной монеты в исходнике много пустого поля и она выглядит меньше соседей — точечный `scale` по id и важные нюансы (не ломать hover на карточке): **`docs/COIN_IMAGES_STRUCTURE.md`** → раздел «Масштаб отображения в UI».
- Сейчас:
