...
- **Каталог монет (Perth и защита от перезаписи):** массовая перезапись Perth-монет одним каноником (Kookaburra и др.) исправлялась скриптом `fix-perth-overwritten-coins.js`. В **update-perth-from-canonical-json.js** сопоставление **только по source_url** — один URL = один продукт, повторений не будет. Записи без source_url этим скриптом не обновляются (нужен импорт с URL или fix-perth-overwritten-coins). В import-perth-mint-to-db.js по-прежнему защита: по catalog_number обновление только если в БД ровно одна запись.
