...
Список id в скрипте: `IDS = ["4611", "5335", "5065", "5837"]` (актуально; прежние проверки — в таблице выше). Чтобы добавить другие монеты Perth, дописать id в `scripts/compare-perth-images.js` и убедиться, что в БД у них заполнен `source_url` (perthmint.com) и есть соответствующий `data/perth-mint-*.json` по тому же URL.
