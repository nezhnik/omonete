...
**Путь на сайте:** `/data/metal-prices.json` (на проде **всегда** создаётся и обновляется **только кроном на сервере**).
...
- Локальный крон Node (`scripts/cron-metal-prices.js`) продолжает писать `public/data/metal-prices.json`, чтобы в dev были актуальные данные.
- При `npm run build` файл `out/data/metal-prices.json` автоматически удаляется скриптом `scripts/clean-metal-prices-from-out.js`, чтобы при заливке папки `out/` на Reg.ru не перезатирать файл, который ведёт серверный крон.
- Сам файл `public/data/metal-prices.json` добавлен в `.gitignore` и **не попадает в репозиторий**: в проде источником истины является только `data/metal-prices.json` на сервере.
