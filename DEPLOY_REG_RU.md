...
**Обычно:** залить всё содержимое папки `out/` в корень сайта на хостинге (например в `www/omonete.ru`).

**Инкрементально (меньше трафика):** после сборки выполнить `npm run regre` — в `out/regre/` появятся только изменённые и новые файлы. Заливать на сервер только содержимое `out/regre/` (см. `docs/DEPLOY_DELTA_REGRE.md`).
