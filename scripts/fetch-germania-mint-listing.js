/**
 * Сбор ссылок монет с Germania Mint: https://germaniamint.com/all-coins/
 *
 * Что делает:
 * - открывает листинг all-coins;
 * - догружает карточки кнопкой Load More и скроллом;
 * - собирает URL карточек монет из блока .alm-listing;
 * - сохраняет:
 *   - scripts/germania-mint-urls.txt (список URL, по одному на строку)
 *   - data/germania-mint-listing-products.json (снимок с title/url)
 *
 * Запуск:
 *   npm run germania:listing
 *   node scripts/fetch-germania-mint-listing.js --full
 */
const fs = require("fs");
const path = require("path");

const DEFAULT_URL = "https://germaniamint.com/all-coins/";
const URL_LIST_FILE = path.join(__dirname, "germania-mint-urls.txt");
const DATA_DIR = path.join(__dirname, "..", "data");
const PRODUCTS_JSON = path.join(DATA_DIR, "germania-mint-listing-products.json");
  ...
  return page.evaluate(() => {
    ...
    wrappers.forEach((item) => {
      ...
      const title =
        ...
        null;
      const obverse =
        item.querySelector("img.coin-face-obverse")?.getAttribute("src") ||
        item.querySelector("img.coin-face-obverse")?.getAttribute("data-src") ||
        null;
      const reverse =
        item.querySelector("img.coin-face-reverse")?.getAttribute("src") ||
        item.querySelector("img.coin-face-reverse")?.getAttribute("data-src") ||
        null;
      if (!map.has(abs)) map.set(abs, { url: abs, title, obverse, reverse });
    });
    ...
    if (map.size === 0) {
      ...
      anchors.forEach((a) => {
        ...
        const title =
          ...
          null;
        if (!map.has(abs)) map.set(abs, { url: abs, title, obverse: null, reverse: null });
      });
    }
...
async function extractPagingUrls(page) {
  ...
  const links = new Set();
  const re = /href="(https:\/\/germaniamint\.com\/all-coins\/\?pg=\d+)"/gi;
  let m;
...
async function main() {
  ...
  console.log("Собрано URL:", urls.length);
