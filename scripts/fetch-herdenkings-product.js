/**
 * Парсинг одной карточки Herdenkingsmunten (FR, Magento).
 */
const fs = require("fs");
...
const IMG_DIR = path.join(ROOT, "public", "image", "coins", "foreign", "herdenkings");
...
async function saveParsed(parsed) {
  ...
  for (let i = 0; i < (parsed.imageUrls || []).length; i++) {
    ...
    const rel = `/image/coins/foreign/herdenkings/${slug}/${fn}`;
  ...
  const outFile = path.join(DATA_DIR, `herdenkings-${slug}.json`);
...
async function main() {
  ...
  if (!rawUrl) {
    console.error('Укажите URL: node scripts/fetch-herdenkings-product.js "https://..."');
    process.exit(1);
  }
