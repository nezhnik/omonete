/**
 * Импорт Australian Kookaburra (1 oz) из data/kookaburra-wikipedia.json в таблицу coins.
 * Данные из Wikipedia: тиражи bullion и proof по годам.
 * По catalog_number дубликаты не создаём.
 *
 * Запуск: node scripts/import-kookaburra-wikipedia-to-db.js
 */
...
const JSON_PATH = path.join(__dirname, "..", "data", "kookaburra-wikipedia.json");
...
async function main() {
  ...
  for (const c of coins) {
    ...
    // Точные тиражи из Wikipedia, без округления
    ...
    const values = [
      ...
      c.metal_fineness || specs.metal_fineness_until_2017 || "999/1000",
      mintage,
      c.weight_g || specs.weight_g || "31,135",
      c.weight_oz || specs.weight_oz || "1 унция",
      releaseDateVal,
      catalogNumber,
      c.quality || "АЦ",
      c.diameter_mm || specs.diameter_mm || "40,6",
      c.thickness_mm || specs.thickness_mm || "4"
    ];
  ...
  await conn.end();
  console.log("✓ Kookaburra (Wikipedia): добавлено", inserted, ", пропущено (уже есть)", skipped);
}
