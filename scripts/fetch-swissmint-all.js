/**
 * Массовый парсинг Swissmint из scripts/swissmint-urls.txt.
 */
const fs = require("fs");
const path = require("path");
const { fetchOneWithPage, normalizeUrl, slugFromUrl } = require("./fetch-swissmint-product.js");

const ROOT = path.join(__dirname, "..");
const DATA_DIR = path.join(ROOT, "data");
const URLS_TXT = path.join(__dirname, "swissmint-urls.txt");

function outPathForUrl(url) {
  return path.join(DATA_DIR, `swissmint-${slugFromUrl(url)}.json`);
}

async function main() {
  const onlyMissing = process.argv.includes("--only-missing");
  if (!fs.existsSync(URLS_TXT)) {
    console.error("Нет файла URL:", URLS_TXT, "Сначала: npm run swissmint:listing");
    process.exit(1);
  }
  const list = fs
