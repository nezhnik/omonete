/**
 * Массовый парсинг Herdenkings из scripts/herdenkings-urls.txt.
 */
const fs = require("fs");
const path = require("path");
const { fetchOneWithPage, normalizeUrl, slugFromUrl } = require("./fetch-herdenkings-product.js");
...
function outPathForUrl(url) {
  return path.join(DATA_DIR, `herdenkings-${slugFromUrl(url)}.json`);
}

async function main() {
  const onlyMissing = process.argv.includes("--only-missing");
  if (!fs.existsSync(URLS_TXT)) {
    console.error("Нет файла URL:", URLS_TXT, "Сначала: npm run herdenkings:listing");
    process.exit(1);
  }
  const list = fs
